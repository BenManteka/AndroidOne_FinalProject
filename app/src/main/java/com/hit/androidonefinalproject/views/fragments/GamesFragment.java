...
public class GamesFragment extends Fragment implements GamesAdapter.GameOnClickListener {
    ...
    private void setDropDownMenu() {
        String[] items = new String[]{"Genre", "Developer", "Name", "Year", "Platform"};
    ...
    private void search() {
        ...
        String selectedCriteria = binding.spinner1.getSelectedItem().toString();
        ...
        for (GameModel game : originalGames) {
            ...
            switch (selectedCriteria) {
                case "Name":
                    match = game.title.toLowerCase().contains(userInput);
                    break;
                case "Genre":
                    match = game.genre.toLowerCase().contains(userInput);
                    break;
                case "Developer":
                    match = game.developer.toLowerCase().contains(userInput);
                    break;
                case "Publisher":
                    match = game.publisher.toLowerCase().contains(userInput);
                    break;
                case "Year":
                    match = game.release_date.contains(userInput);
                    break;
                case "Platform":
                    match = game.platform.toLowerCase().contains(userInput);
                    break;
                default: // מבצע סינון מהרשימה המקורית, ולא מהרשימה
                    match = game.title.toLowerCase().contains(userInput) ||
